...
public class EvolutionManager : MonoBehaviour
{
    ...
    private void InitializePopulation()
    {
        ...
        for (int i = 0; i < populationSize; i++)
        {
            ...
            shape.wingSpan = Random.Range(1f, 200f); // 翼幅をランダムに設定
            shape.wingLength = Random.Range(0.5f, 2.5f); // 翼の長さをランダムに設定
            shape.wingAngle = Random.Range(-20f, 20f); // 翼角度をランダムに設定
    ...
    private GameObject Mutate(GameObject plane)
    {
        ...
        if (Random.value < mutationRate)
        {
            ...
            shape.wingSpan = Mathf.Clamp(shape.wingSpan, 1f, 200f); // 翼幅の範囲制限
        }

        if (Random.value < mutationRate)
        {
            shape.wingLength += Random.Range(-0.4f, 0.4f); // 翼の長さの突然変異
            shape.wingLength = Mathf.Clamp(shape.wingLength, 0.5f, 2.5f); // 翼の長さの範囲制限
        }

        if (Random.value < mutationRate)
        {
            shape.wingAngle += Random.Range(-1f, 1f); // 翼角度の突然変異
            shape.wingAngle = Mathf.Clamp(shape.wingAngle, -20f, 20f); // 翼角度の範囲制限
        }
